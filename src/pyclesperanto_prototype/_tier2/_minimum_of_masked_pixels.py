from .._tier0 import (Image, create, create_2d_xy, create_2d_yx, execute,
                      plugin_function, pull)
from .._tier1 import copy_slice, transpose_xz, transpose_yz


def __minimum_of_masked_pixels_reduction(
    source: Image, source_mask: Image, output: Image, output_mask: Image
) -> Image:
    parameters = {
        "dst_min": output,
    ...
    execute(
        __file__,
        "clij-opencl-kernels/kernels/minimum_of_masked_pixels_3d_2d_x.cl",
        "minimum_of_masked_pixels_3d_2d",
        output.shape,
        parameters,
    )
...
def minimum_of_masked_pixels(source: Image, mask: Image):
    ...
    if len(dimensionality) == 2:  # 2D image
        ...
        __minimum_of_masked_pixels_reduction(
            temp_input, temp_mask, reduced_image, reduced_mask
        )
