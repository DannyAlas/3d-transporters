...
def sub_stack(
    source: Image, destination: Image = None, start_z: int = 0, end_z: int = 0
) -> Image:
    ...
    if num_slices < 1:
        warn(
            "In sub_stack, end_z should be equal or larger than start_z as num slices must be at least 1"
        )
        num_slices = 1

    if len(dims) < 3:
        warn(
            "A 2D image was passed to sub_stack, which is meant to process 3D images. It will just be copied."
        )
