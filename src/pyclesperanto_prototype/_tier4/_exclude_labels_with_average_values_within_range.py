...
def exclude_labels_with_average_values_within_range(
    values_image: Image,
    ...
    statistics = statistics_of_background_and_labelled_pixels(
        values_image, label_map_input
    )
    values_vector = np.asarray([statistics["mean_intensity"]])
    from .._tier3 import exclude_labels_with_values_within_range

    return exclude_labels_with_values_within_range(
        values_vector,
        label_map_input,
        label_map_destination,
        minimum_value_range,
