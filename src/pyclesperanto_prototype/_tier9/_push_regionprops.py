...
def push_regionprops(
    props: Union[dict, RegionProperties], first_row_index: int = 0
) -> Image:
    """
    ...
    if isinstance(props, dict):
        ...
        for j, label_props in enumerate(props):
            ...
            if i >= first_row_index:
                ...
                matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_WIDTH.value] = (
                    matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_END_X.value]
                    - matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_X.value]
                    + 1
                )
                matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_HEIGHT.value] = (
                    matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_END_Y.value]
                    - matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_Y.value]
                    + 1
                )
                matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_DEPTH.value] = (
                    matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_END_Z.value]
                    - matrix[i][STATISTICS_ENTRY.BOUNDING_BOX_Z.value]
                    + 1
                )

                # intensity measurements
                matrix[i][
                    STATISTICS_ENTRY.MINIMUM_INTENSITY.value
                ] = label_props.min_intensity
                matrix[i][
                    STATISTICS_ENTRY.MAXIMUM_INTENSITY.value
                ] = label_props.max_intensity
                matrix[i][
                    STATISTICS_ENTRY.MEAN_INTENSITY.value
                ] = label_props.mean_intensity
                # sum intensity
                matrix[i][STATISTICS_ENTRY.SUM_INTENSITY.value] = (
                    label_props.mean_intensity * label_props.area
                )
                # stddev intensity
                if hasattr(label_props, "standard_deviation_intensity"):
                    matrix[i][
                        STATISTICS_ENTRY.STANDARD_DEVIATION_INTENSITY.value
                    ] = label_props.standard_deviation_intensity
                else:
                    warn(
                        "regionprops didn't contains stanard_deviation_intensity. Consider using cle.statistics_of_labelled_pixels to determine the standard deviation. "
                    )
                ...
                if len(label_props.weighted_centroid) == 3:
                    matrix[i][
                        STATISTICS_ENTRY.MASS_CENTER_X.value
                    ] = label_props.weighted_centroid[2]
                    matrix[i][
                        STATISTICS_ENTRY.MASS_CENTER_Y.value
                    ] = label_props.weighted_centroid[1]
                    matrix[i][
                        STATISTICS_ENTRY.MASS_CENTER_Z.value
                    ] = label_props.weighted_centroid[0]
                else:
                    matrix[i][
                        STATISTICS_ENTRY.MASS_CENTER_X.value
                    ] = label_props.weighted_centroid[1]
                ...
                if len(label_props.centroid) == 3:
                    matrix[i][STATISTICS_ENTRY.CENTROID_X.value] = label_props.centroid[
                        2
                    ]
                    matrix[i][STATISTICS_ENTRY.CENTROID_Y.value] = label_props.centroid[
                        1
                    ]
                    matrix[i][STATISTICS_ENTRY.CENTROID_Z.value] = label_props.centroid[
                        0
                    ]
                else:
                    matrix[i][STATISTICS_ENTRY.CENTROID_X.value] = label_props.centroid[
                ...
                if hasattr(label_props, "sum_distance_to_mass_center"):
                    matrix[i][
                        STATISTICS_ENTRY.SUM_DISTANCE_TO_MASS_CENTER.value
                    ] = label_props["sum_distance_to_mass_center"]
                if hasattr(label_props, "mean_distance_to_mass_center"):
                    matrix[i][
                        STATISTICS_ENTRY.MEAN_DISTANCE_TO_MASS_CENTER.value
                    ] = label_props["mean_distance_to_mass_center"]
                if hasattr(label_props, "max_distance_to_mass_center"):
                    matrix[i][
                        STATISTICS_ENTRY.MAX_DISTANCE_TO_MASS_CENTER.value
                    ] = label_props["max_distance_to_mass_center"]
                if hasattr(label_props, "mean_max_distance_to_mass_center_ratio"):
                    matrix[i][
                        STATISTICS_ENTRY.MAX_MEAN_DISTANCE_TO_MASS_CENTER_RATIO.value
                    ] = label_props["mean_max_distance_to_mass_center_ratio"]

                if hasattr(label_props, "sum_distance_to_centroid"):
                    matrix[i][
                        STATISTICS_ENTRY.SUM_DISTANCE_TO_CENTROID.value
                    ] = label_props["sum_distance_to_centroid"]
                if hasattr(label_props, "mean_distance_to_centroid"):
                    matrix[i][
                        STATISTICS_ENTRY.MEAN_DISTANCE_TO_CENTROID.value
                    ] = label_props["mean_distance_to_centroid"]
                if hasattr(label_props, "max_distance_to_centroid"):
                    matrix[i][
                        STATISTICS_ENTRY.MAX_DISTANCE_TO_CENTROID.value
                    ] = label_props["max_distance_to_centroid"]
                if hasattr(label_props, "mean_max_distance_to_centroid_ratio"):
                    matrix[i][
                        STATISTICS_ENTRY.MAX_MEAN_DISTANCE_TO_CENTROID_RATIO.value
                    ] = label_props["mean_max_distance_to_centroid_ratio"]
