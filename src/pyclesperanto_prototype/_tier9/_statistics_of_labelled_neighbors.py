...
def statistics_of_labelled_neighbors(
    ...
    * This operation assumes input images are isotropic.
    ...
    pandas.DataFrame
    """
    from .._tier1 import (average_distance_of_n_far_off_distances,
                          average_distance_of_n_nearest_distances,
                          average_distance_of_touching_neighbors, copy,
                          count_touching_neighbors, generate_distance_matrix,
    ...
    all_stats["touching_neighbor_count"] = cle_to_numpy(
        remove_first=True, data=count_touching_neighbors(touch_matrix)
    )

    # print("tnc", all_stats["touching_neighbor_count"])

    # distances of touching neighbors
    all_stats["minimum_distance_of_touching_neighbors"] = cle_to_numpy(
        remove_first=True,
        data=minimum_distance_of_touching_neighbors(distance_matrix, touch_matrix),
    )
    all_stats["average_distance_of_touching_neighbors"] = cle_to_numpy(
        remove_first=True,
        data=average_distance_of_touching_neighbors(distance_matrix, touch_matrix),
    )
    all_stats["maximum_distance_of_touching_neighbors"] = cle_to_numpy(
        remove_first=True,
        data=maximum_distance_of_touching_neighbors(distance_matrix, touch_matrix),
    )
    ...
    with warnings.catch_warnings():
        ...
        all_stats["max_min_distance_ratio_of_touching_neighbors"] = (
            all_stats["maximum_distance_of_touching_neighbors"]
    ...
    for d in proximal_distances:
        proximal_touch_matrix = generate_proximal_neighbors_matrix(
            distance_matrix, min_distance=0, max_distance=d
        )
        # print("proximal_touch_matrix", d,"\n", proximal_touch_matrix)

        all_stats["proximal_neighbor_count_d" + str(d)] = cle_to_numpy(
            remove_first=True, data=count_touching_neighbors(proximal_touch_matrix)
        )
    ...
    for n in nearest_neighbor_ns:
        # max distance
        all_stats[
            "maximum_distance_of_n" + str(n) + "_nearest_neighbors"
        ] = cle_to_numpy(
            remove_first=True,
            data=maximum_distance_of_n_shortest_distances(
                self_far_distance_matrix, n=n
            ),
        )

        # mean distance
        all_stats[
            "average_distance_of_n" + str(n) + "_nearest_neighbors"
        ] = cle_to_numpy(
            remove_first=True,
            data=average_distance_of_n_nearest_distances(self_far_distance_matrix, n=n),
        )

    all_stats["distance_to_most_distant_other"] = cle_to_numpy(
        remove_first=True,
        data=average_distance_of_n_far_off_distances(distance_matrix, n=1),
    )
    ...
    touch_count_matrix_0_set_to_high = replace_intensity(
        touch_count_matrix,
        value_to_replace=0,
    ...
    for touch_portion_threshold in [0, 0.16, 0.2, 0.33, 0.5, 0.75]:
        touch_portion_within_range_neighbors_matrix = (
            generate_touch_portion_within_range_neighbors_matrix(
                touch_portion_matrix, minimum_touch_portion=touch_portion_threshold
            )
        )
        ...
        all_stats[
            "touch_portion_above_" + str(touch_portion_threshold) + "_neighbor_count"
        ] = cle_to_numpy(
            remove_first=True,
            data=count_touching_neighbors(touch_portion_within_range_neighbors_matrix),
        )
    ...
    touch_portion_matrix_0_set_to_high = replace_intensity(
        temp, value_to_replace=0, value_replacement=temp.max()
    )

    # number of pixels where the label touches others; excluding image borders
    all_stats["touch_count_sum"] = cle_to_numpy(
        remove_first=True, data=sum_y_projection(touch_count_matrix)
    )
    all_stats["minimum_touch_count"] = cle_to_numpy(
        remove_first=True, data=minimum_y_projection(touch_count_matrix_0_set_to_high)
    )
    all_stats["maximum_touch_count"] = cle_to_numpy(
        remove_first=True, data=maximum_y_projection(touch_count_matrix)
    )

    all_stats["minimum_touch_portion"] = cle_to_numpy(
        remove_first=True, data=minimum_y_projection(touch_portion_matrix_0_set_to_high)
    )
    all_stats["maximum_touch_portion"] = cle_to_numpy(
        remove_first=True, data=maximum_y_projection(touch_portion_matrix)
    )

    # todo: this might be possible more efficiently by refactoring; at the moment it takes the label image
    all_stats["standard_deviation_touch_portion"] = cle_to_numpy(
        remove_first=True, data=standard_deviation_touch_portion(label_image)
    )
    ...
    for dilation_radius in dilation_radii:
        ...
        dilated_distance_matrix = generate_distance_matrix(
            dilated_centroids, dilated_centroids
        )
        # print("distance matrix", distance_matrix.shape)
        ...
        all_stats[
            "touching_neighbor_count_dilated_r_" + str(dilation_radius)
        ] = cle_to_numpy(
            remove_first=True, data=count_touching_neighbors(dilated_touch_matrix)
        )
        ...
        # maximum_distance_of_touching_neighbors,
        all_stats[
            "minimum_distance_of_touching_neighbors_dilated_r_" + str(dilation_radius)
        ] = cle_to_numpy(
            remove_first=True,
        ...
        all_stats[
            "average_distance_of_touching_neighbors_dilated_r_" + str(dilation_radius)
        ] = cle_to_numpy(
            remove_first=True,
        ...
        all_stats[
            "maximum_distance_of_touching_neighbors_dilated_r_" + str(dilation_radius)
        ] = cle_to_numpy(
            remove_first=True,
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            all_stats[
                "max_min_distance_ratio_of_touching_neighbors_dilated_r_"
                + str(dilation_radius)
            ] = (
                all_stats[
                    "maximum_distance_of_touching_neighbors_dilated_r_"
                    + str(dilation_radius)
                ]
                / all_stats[
                    "minimum_distance_of_touching_neighbors_dilated_r_"
                    + str(dilation_radius)
                ]
            )
        ...
        dilated_touch_portion_matrix = generate_touch_portion_matrix(
            dilated_label_image
        )
        # ignore touching background
        ...
        # To detect the minimum touch count/portion, we need to set all 0 pixels to a high value
        dilated_touch_count_matrix_0_set_to_high = replace_intensity(
            dilated_touch_count_matrix,
            value_to_replace=0,
        ...
        all_stats["touch_count_sum_dilated_r_" + str(dilation_radius)] = cle_to_numpy(
            remove_first=True, data=sum_y_projection(dilated_touch_count_matrix)
        )
        all_stats[
            "minimum_touch_count_dilated_r_" + str(dilation_radius)
        ] = cle_to_numpy(
            remove_first=True,
            data=minimum_y_projection(dilated_touch_count_matrix_0_set_to_high),
        )
        all_stats[
            "maximum_touch_count_dilated_r_" + str(dilation_radius)
        ] = cle_to_numpy(
            remove_first=True, data=maximum_y_projection(dilated_touch_count_matrix)
        )

        all_stats[
            "minimum_touch_portion_dilated_r_" + str(dilation_radius)
        ] = cle_to_numpy(
            remove_first=True,
            data=minimum_y_projection(dilated_touch_portion_matrix_0_set_to_high),
        )
        all_stats[
            "maximum_touch_portion_dilated_r_" + str(dilation_radius)
        ] = cle_to_numpy(
            remove_first=True, data=maximum_y_projection(dilated_touch_portion_matrix)
        )
