...
from .._tier0 import (Image, create, create_like, create_none, plugin_function,
                      push)
...
def affine_transform(
    ...
    from .._tier0 import create, empty_image_like, execute
    from .._tier1 import copy, copy_slice
    ...
    execute(
        __file__,
        "clij-opencl-kernels/kernels/affine_transform_"
        + str(len(destination.shape))
        + "d"
        + kernel_suffix
        + "_x.cl",
        "affine_transform_" + str(len(destination.shape)) + "d" + kernel_suffix,
        destination.shape,
        parameters,
    )
...
def _determine_translation_and_bounding_box(
    source: Image, affine_transformation: AffineTransform3D
):
    ...
    new_shape = (
        np.around((max_coordinate - min_coordinate)[0:3]).astype(int).tolist()[::-1]
    )
