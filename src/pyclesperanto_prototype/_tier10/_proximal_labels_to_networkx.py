...
def proximal_labels_to_networkx(
    label_image: Image,
    minimum_distance: float = 0,
    maximum_distance: float = np.finfo(np.float32).max,
):
    """
    ...
    proximal_matrix = generate_proximal_neighbors_matrix(
        distance_matrix, min_distance=minimum_distance, max_distance=maximum_distance
    )
