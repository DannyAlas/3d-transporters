...
from .._tier0 import Image, create_labels_like, create_none, plugin_function


@plugin_function(
    categories=["label processing", "in assistant"], output_creator=create_labels_like
)
def merge_labels_with_border_intensity_within_range(
    image: Image,
    labels: Image,
    ...
    * The specified range includes minimum and maximum
    ...
    touch_matrix = generate_touch_mean_intensity_within_range_matrix(
        image,
        labels,
    ...
    labels_destination = merge_labels_according_to_touch_matrix(
        labels, touch_matrix, labels_destination
    )

    # determine matrix again and see if there is anything to be merged
    new_matrix = generate_touch_mean_intensity_within_range_matrix(
        image,
        labels_destination,
        minimum_intensity=minimum_intensity,
        maximum_intensity=maximum_intensity,
    ...
    if sum_of_all_pixels(new_matrix) > 0:
        ...
        return merge_labels_with_border_intensity_within_range(
            image,
            copied,
            labels_destination,
            minimum_intensity=minimum_intensity,
