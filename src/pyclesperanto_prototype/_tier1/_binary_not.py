...
@plugin_function(
    categories=["binary processing", "filter", "label processing", "in assistant"],
    output_creator=create_binary_like,
)
def binary_not(source: Image, destination: Image = None) -> Image:
    ...
    parameters = {"src1": source, "dst": destination}

    execute(
        __file__,
        "clij-opencl-kernels/kernels/binary_not_"
        + str(len(destination.shape))
        + "d_x.cl",
        "binary_not_" + str(len(destination.shape)) + "d",
        destination.shape,
        parameters,
    )
    return destination
