...
def generate_touch_matrix(
    label_map: Image, touch_matrix_destination: Image = None
) -> Image:
    ...
    parameters = {"dst_matrix": touch_matrix_destination, "src_label_map": label_map}

    execute(
        __file__,
        "generate_touch_matrix_" + str(len(label_map.shape)) + "d_x.cl",
        "generate_touch_matrix_" + str(len(label_map.shape)) + "d",
        label_map.shape,
        parameters,
    )
