...
def multiply_images(factor1: Image, factor2: Image, destination: Image = None) -> Image:
    ...
    parameters = {"src": factor1, "src1": factor2, "dst": destination}

    execute(
        __file__,
        "clij-opencl-kernels/kernels/multiply_images_"
        + str(len(destination.shape))
        + "d_x.cl",
        "multiply_images_" + str(len(destination.shape)) + "d",
        destination.shape,
        parameters,
    )
