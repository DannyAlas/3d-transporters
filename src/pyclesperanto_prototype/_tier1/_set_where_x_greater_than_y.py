...
@plugin_function
def set_where_x_greater_than_y(source : Image, value : float = 0) -> Image:
    ...
    execute(__file__, 'clij-opencl-kernels/kernels/set_where_x_greater_than_y_' + str(len(source.shape)) + 'd_x.cl', 'set_where_x_greater_than_y_' + str(len(source.shape)) + 'd', source.shape, parameters)
    return source
