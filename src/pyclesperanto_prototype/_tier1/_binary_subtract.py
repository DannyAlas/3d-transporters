...
def binary_subtract(
    minuend: Image, subtrahend: Image, destination: Image = None
) -> Image:
    ...
    parameters = {"dst": destination, "src1": minuend, "src2": subtrahend}
    execute(
        __file__,
        "clij-opencl-kernels/kernels/binary_subtract_"
        + str(len(destination.shape))
        + "d_x.cl",
        "binary_subtract_" + str(len(destination.shape)) + "d",
        destination.shape,
        parameters,
    )
    return destination
