...
def maximum_image_and_scalar(
    ...
    parameters = {"src": source, "dst": destination, "valueB": float(scalar)}

    execute(
        __file__,
        "clij-opencl-kernels/kernels/maximum_image_and_scalar_"
        + str(len(destination.shape))
        + "d_x.cl",
        "maximum_image_and_scalar_" + str(len(destination.shape)) + "d",
        destination.shape,
        parameters,
    )
    return destination
