...
def convolve(
    source: Image, convolution_kernel: Image, destination: Image = None
) -> Image:
    ...
    parameters = {"src": source, "kernelImage": convolution_kernel, "dst": destination}

    execute(
        __file__,
        "clij-opencl-kernels/kernels/convolve_"
        + str(len(destination.shape))
        + "d_x.cl",
        "convolve_" + str(len(destination.shape)) + "d",
        destination.shape,
        parameters,
    )
    return destination
