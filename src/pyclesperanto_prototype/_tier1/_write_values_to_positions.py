from .._tier0 import Image, create, create_none, execute, plugin_function, pull
from .._tier1 import maximum_x_projection


@plugin_function(output_creator=create_none)
def write_values_to_positions(
    positions_and_values: Image, destination: Image = None
) -> Image:
    ...
    parameters = {"dst": destination, "src": positions_and_values}

    execute(
        __file__,
        "clij-opencl-kernels/kernels/write_values_to_positions_"
        + str(len(destination.shape))
        + "d_x.cl",
        "write_values_to_positions_" + str(len(destination.shape)) + "d",
        positions_and_values.shape,
        parameters,
    )
    return destination
