...
def subtract_image_from_scalar(
    source: Image, destination: Image = None, scalar: float = 0
) -> Image:
    ...
    parameters = {"src": source, "dst": destination, "scalar": float(scalar)}
    execute(
        __file__,
        "clij-opencl-kernels/kernels/subtract_image_from_scalar_"
        + str(len(destination.shape))
        + "d_x.cl",
        "subtract_image_from_scalar_" + str(len(destination.shape)) + "d",
        destination.shape,
        parameters,
    )
    return destination
