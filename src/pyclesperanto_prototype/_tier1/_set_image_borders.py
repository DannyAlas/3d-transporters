...
@plugin_function
def set_image_borders(destination: Image, value: float = 0) -> Image:
    ...
    parameters = {"dst": destination, "value": float(value)}

    execute(
        __file__,
        "clij-opencl-kernels/kernels/set_image_borders_"
        + str(len(destination.shape))
        + "d_x.cl",
        "set_image_borders_" + str(len(destination.shape)) + "d",
        destination.shape,
        parameters,
    )
    return destination
