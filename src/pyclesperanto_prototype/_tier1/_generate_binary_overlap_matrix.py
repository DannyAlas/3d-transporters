from .._tier0 import (Image, create_square_matrix_from_two_labelmaps, execute,
                      plugin_function)
...
def generate_binary_overlap_matrix(
    label_map1: Image,
    label_map2: Image,
    binary_overlap_matrix_destination: Image = None,
) -> Image:
    ...
    execute(
        __file__,
        "clij-opencl-kernels/kernels/generate_binary_overlap_matrix_"
        + str(len(label_map1.shape))
        + "d_x.cl",
        "generate_binary_overlap_matrix_" + str(len(label_map1.shape)) + "d",
        label_map1.shape,
        parameters,
    )
