...
def replace_intensities(
    source: Image, new_values_vector: Image, destination: Image = None
) -> Image:
    ...
    parameters = {"dst": destination, "src": source, "map": new_values_vector}

    execute(
        __file__,
        "clij-opencl-kernels/kernels/replace_intensities_x.cl",
        "replace_intensities",
        destination.shape,
        parameters,
    )
