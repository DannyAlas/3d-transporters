...
def power(source: Image, destination: Image = None, exponent: float = 1) -> Image:
    ...
    parameters = {"src": source, "dst": destination, "exponent": float(exponent)}

    execute(
        __file__,
        "clij-opencl-kernels/kernels/power_" + str(len(destination.shape)) + "d_x.cl",
        "power_" + str(len(destination.shape)) + "d",
        destination.shape,
        parameters,
    )
    return destination
