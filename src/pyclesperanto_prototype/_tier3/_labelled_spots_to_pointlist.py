from .._tier0 import (Image, create_pointlist_from_labelmap, execute,
                      plugin_function)
...
def labelled_spots_to_pointlist(
    input_labelled_spots: Image, destination_pointlist: Image = None
) -> Image:
    ...
    parameters = {"src": input_labelled_spots, "dst_point_list": destination_pointlist}

    # todo: make naming conventions fit
    execute(
        __file__,
        "clij-opencl-kernels/kernels/labelled_spots_to_point_list_x.cl",
        "labelled_spots_to_point_list",
        input_labelled_spots.shape,
        parameters,
    )
