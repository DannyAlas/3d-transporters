...
from pyclesperanto_prototype._tier0 import Image, execute, plugin_function

from .._tier0 import create_labels_like, create_like, pull, push
from .._tier1 import replace_intensities
from .._tier2 import maximum_of_all_pixels


@plugin_function(
    categories=["label processing", "in assistant"],
    priority=1,
...
def exclude_labels_on_edges(
    ...
    exlude_in_x: bool = True,
    exlude_in_y: bool = True,
    exlude_in_z: bool = True,
) -> Image:
    ...
    parameters = {"src": label_map_input, "label_index_dst": label_index_map}
    if len(label_map_input.shape) == 3:
        ...
        dimensions = [1, label_map_input.shape[0], label_map_input.shape[1]]

    if len(label_map_input.shape) == 3 and exlude_in_z:
        global_sizes = [1, dimensions[1], dimensions[2]]
        execute(
            __file__,
            "clij-opencl-kernels/kernels/exclude_labels_on_edges_3d_x.cl",
            "exclude_on_edges_z_3d",
            global_sizes,
            parameters,
        )
