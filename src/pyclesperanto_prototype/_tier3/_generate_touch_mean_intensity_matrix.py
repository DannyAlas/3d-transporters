...
def generate_touch_mean_intensity_matrix(
    intensity_image: Image,
    label_map: Image,
    touch_mean_intensity_matrix_destination: Image = None,
) -> Image:
    ...
    * The intensity_image should be of integer type. In case of float images, information might be lost.
    ...
    """
    if intensity_image.dtype not in [
        np.uint8,
        np.int8,
        np.uint16,
        np.int16,
        ...
        warnings.warn(
            "generate_touch_mean_intensity_matrix is supposed to work with images of integer type only.\n"
            + "Loss of information is possible when passing non-integer images."
        )

    from .._tier0 import create, execute
    from .._tier1 import divide_images, set
    ...
    return divide_images(
        touch_intensity_sum, touch_count, touch_mean_intensity_matrix_destination
    )
